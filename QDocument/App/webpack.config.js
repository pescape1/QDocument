...
if (config.appSettings != null) {
    ...
    fileSave(path.join(path.resolve(__dirname, "../wwwroot/dist"), "app-settings.js"))
...
const webpathConfig = {
    ...
    entry: path.resolve(__dirname, 'src/app.js'),     // Specifies the entry point of the application where webpack begins the packaging process.  
    output: {
        path: path.resolve(__dirname, '../wwwroot/dist'),  // Specifies the output directory for any wepback output files
        filename: "main.js",                              // Specifies the file name template for entry chunks (TODO: figure out what an entry point chunk is),
